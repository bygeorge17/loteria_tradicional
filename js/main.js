...
new Vue({
    ...
    methods:{
            ...
            if (this.cartas.length<2) {
                ...
                if (this.barajas.length!=0) {
                    ...
                    for (let index = 0; index < this.cartas.length; index++) {
                        for (let index1 = 0; index1 < this.cartas[index].carta.length; index1++) {
                            if (this.cartas[index].carta[index1].baraja==this.actual) {
                                this.cartas[index].carta[index1].paso=true;
                            }
                        }
                        if (!this.cartas[index].carta.some(baraja=>baraja.paso==false)) {
        ...
        nuevoJuego:function(){
            let nuevo=confirm('Esta seguro de que quiere crear un juego nuevo, LOS DATOS ACTUALES SE PERDERAN');
            if (nuevo) {
                this.barajas=[];
                this.barajar();
               this.cartas=[];
